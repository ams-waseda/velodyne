...
namespace velodyne_rawdata
{
                             ...
                             VPointCloud &pc)
  {
    ...
    const raw_packet_t *raw = (const raw_packet_t *) &pkt.data[0];
    uint8_t mode = raw->status[0]-0x36;
    //ROS_INFO("VLP16 mode=%x hw=%x\n", raw->status[0],raw->status[1] );
    
    int dblock=mode==VLP16_MODE_DUAL?2:1;
    for (int block = 0; block < BLOCKS_PER_PACKET; block+=dblock) {
      ...
      if (block < (BLOCKS_PER_PACKET-dblock)){
        azimuth_diff = (float)((36000 + raw->blocks[block+dblock].rotation - raw->blocks[block].rotation)%36000);
      ...
      for (int firing=0, k=0; firing < VLP16_FIRINGS_PER_BLOCK; firing++){
        for (int dsr=0; dsr < VLP16_SCANS_PER_FIRING; dsr++, k+=RAW_SCAN_SIZE){
          ...
          /** Time Stamp **/
          timestamp = raw->time+(uint32_t)((block/dblock)*VLP16_BLOCK_TDURATION+firing*VLP16_FIRING_TOFFSET+dsr*VLP16_DSR_TOFFSET);
            ...
            if ((azimuth_corrected >= config_.min_angle 
              ...
              if (echocnt==0) {
                ...
                if (pointInRange(distance)) {
                  ...
                  point.mode = (float)(mode==VLP16_MODE_DUAL?VLP16_MODE_LAST:mode);
                ...
                float ddistance = distance-last_distance;
                if (pointInRange(distance)) {
                  if (fabs(ddistance)>0.1) {
                    // append this point to the cloud
                    VPoint point;
                    point.ring = (float)corrections.laser_ring;
                    point.x = x_coord;
                    ...
                    point.intensity = (uint8_t) intensity;
                    point.time = timestamp;
                    point.mode = (float)(VLP16_MODE_STRONGEST);
                    point.azimuth = azimuth_corrected_f/100;
                    point.distance = distance;
